                ...
                @foreach($categories as $key => $category)
                    <tr>
                        ...
                        <td class="text-right">
                            ...
                            <a class="btn btn-soft-primary btn-icon btn-circle btn-sm" href="{{route('categories.config', ['id'=>$category->id, 'lang'=>env('DEFAULT_LANGUAGE')] )}}" title="{{ translate('Edit') }}">
    ...
    <script type="text/javascript">
        ...
        function update_published(el){
            ...
            $.post('{{ route('categories.published') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
                    AIZ.plugins.notify('success', '{{ translate('Categories updated successfully') }}');
                }
                else{
                    AIZ.plugins.notify('danger', '{{ translate('Something went wrong') }}');
                }
