...
<div class="aiz-titlebar text-left mt-2 mb-3">
    <div class="row align-items-center">
        ...
        <div class="col-md-6 text-md-right">
            <a href="{{ route('subsubcategories.create') }}" class="btn btn-primary">
                <span>{{translate('Add New Sub Sub Category')}}</span>
            </a>
        </div>
    </div>
</div>
            ...
            <tbody>
                ...
                @foreach($categories as $key => $category)
                    <tr>
                        ...
                        <td class="text-right">
                            <a class="btn btn-soft-primary btn-icon btn-circle btn-sm" href="{{route('subsubcategories.edit', ['id'=>$category->id, 'lang'=>env('DEFAULT_LANGUAGE')] )}}" title="{{ translate('Edit') }}">
                                <i class="las la-edit"></i>
                            </a>
                            <a href="#" class="btn btn-soft-danger btn-icon btn-circle btn-sm confirm-delete" data-href="{{route('subsubcategories.destroy', $category->id)}}" title="{{ translate('Delete') }}">
                                <i class="las la-trash"></i>
                            </a>
                            <a class="btn btn-soft-primary btn-icon btn-circle btn-sm" href="{{route('subsubcategories.config', ['id'=>$category->id, 'lang'=>env('DEFAULT_LANGUAGE')] )}}" title="{{ translate('Edit') }}">
    ...
    <script type="text/javascript">
        function update_featured(el){
            ...
            $.post('{{ route('subsubcategories.featured') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
                    AIZ.plugins.notify('success', '{{ translate('SubSubcategories updated successfully') }}');
                }
                else{
        ...
        function update_published(el){
            ...
            $.post('{{ route('subsubcategories.published') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
