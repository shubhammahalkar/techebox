...
<div class="row">
	<div class="col-xl-10 mx-auto">
		...
		<div class="card">
			<div class="card-header">
				...
				<input type="hidden" name="types[]" value="home_slider">
				<input type="hidden" name="types[]" value="">
                <div class="custom-control custom-switch float-right">
                    <input type="checkbox" onchange="settings_section_activation(this)" value="home_slider_pub" class="custom-control-input" <?php if ($business_settings->where('type','home_slider_pub')->first()->value == 1 )echo "checked";?> id="customSwitches">
                    <label class="custom-control-label" for="customSwitches"> Published </label>
                </div>
			</div>
			<div class="card-body">
		...
		<div class="card">
			<div class="card-header">
                ...
                <div class="custom-control custom-switch float-right">
                    <input type="checkbox" onchange="settings_section_activation(this)" value="section2" class="custom-control-input" <?php if ($business_settings->where('type','section2')->first()->value == 1 )echo "checked";?> >
		...
		<div class="card">
			<div class="card-header">
                ...
                <div class="custom-control custom-switch float-right">
                    <input type="checkbox" onchange="settings_section_activation(this)" value="section5" class="custom-control-input" <?php if ($business_settings->where('type','section5')->first()->value == 1 )echo "checked";?> >
                    <label class="custom-control-label" > Published </label>
                </div>
			</div>
			<div class="card-body">
		...
		<div class="card">
			<div class="card-header">
				<h6 class="mb-0">{{ __('Flash Deals  ') }}</h6>
			</div>
			<div class="card-body">
				<form action="{{ route('business_settings.update') }}" method="POST" enctype="multipart/form-data">
					@csrf
					<div class="form-group">
						{{-- <label>{{ translate('Features') }}</label> --}}
						<div class="home-banner1-target">
							<input type="hidden" name="types[]" value="flash_deals">
							...
							@if (get_setting('flash_deals') != null)
								@foreach (App\Models\Product::whereIn('id',json_decode(get_setting('flash_deals'), true))->get() as $key => $value)
									<div class="row gutters-5">
										<div class="col-md-5">
											<div class="form-group">
                                                <input type="hidden" name="types[]" value="flash_deals">
                                                <input type="hidden" name="flash_deals[]" value="{{$value->id}}">
												<input type="text" class="form-control"    value=" {{$value->name}}">
							...
							<div class="row gutters-5">
								<div class="col-md-5">
									<div class="form-group row" id="category">
                                        ...
                                        <div class="col-md-8">
                                            <input type="hidden" name="types[]" value="flash_deals">
                                            <select class="form-control aiz-selectpicker" name="flash_deals[]" id="category_id" data-live-search="true" required multiple>
                                                <option value="0">---Select Category ---</option>
                                                @foreach (\App\Models\Product::where('published',1)->get() as $product)
                                                <option value="{{ $product->id }}">{{ $product->getTranslation('name') }}</option>
                                                @endforeach
    ...
    <div class="card ml-4">
        <form action="{{ route('business_settings.update') }}" method="POST" enctype="multipart/form-data">
            @csrf
        <div class="card-header">
            <h5 class="mb-0 h6">{{translate('SEO Site Map')}}</h5>
        </div>
        <div class="card-body">
            <div class="form-group row">
                <label class="col-md-3 col-from-label">{{translate(' SEO Site Map')}}</label>
                <div class="col-md-8">
                    <input type="hidden" name="types[]" value="site_map">
                    <textarea class="aiz-text-editor" value="{{get_setting('site_map'), true[$key] }}" name="site_map[]"></textarea>
                </div>
            </div>
            <div class="text-right">
                <button type="submit" class="btn btn-primary">{{ translate('Update') }}</button>
...
@section('script')
    <script type="text/javascript">
        ...
        function settings_section_activation(el) {
                    var status = '1';
                    if (el.checked) {
                        status = '1';
                    } else {
                    ...
                    $.post('{{ route('home_settings_section_activation.update_status') }}', {
                        _token: '{{ csrf_token() }}',
                        id: el.value,
                        status: status
                    }, function (data) {
                        if (data == 1) {
                            showAlert('success', 'Home Page Section Status updated successfully');
                        } else {
                            showAlert('danger', 'There is some error');
                        }
                    });
                }
    </script>
