...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        // // });
         ...
         $todays_deal_products = Product::where('published', 1)->where('todays_deal', '1')->get();

         $flash_deal = new FlashDeal();
         $home_sliders =BusinessSetting::where('type','home_slider_images')->get();
         $user = Auth::user();
         if (!empty($user)) {
        $uid = $user->id;
           ...
           return view('frontend.index', compact('uid'), compact('home_sliders','featured_categories', 'todays_deal_products','business_settings','flash_deal'));
    }
    else{
        // $home_settings['section1'] = BusinessSetting::where('type', 'home_settings_section1')->first();

         return view('frontend.index', compact('home_sliders','featured_categories', 'todays_deal_products','business_settings','flash_deal'));
    }


}

public function home_settings_section_activation(Request $request)
{
    $business_settings = BusinessSetting::where('type', $request->id)->first();
    $business_settings->value = $request->status;
    ...
    {
        $detailedProduct  = Product::with('reviews', 'brand', 'stocks', 'user', 'user.shop')->where('slug', $slug)->first();
        $recent_view = RecentViewProduct::all();
        $categoryOffers = CategoryOffer::where('category_id', 11 )->get();
        if($detailedProduct != null && $detailedProduct->published){
            $pid = $detailedProduct->id;
            $user = Auth::user();
             $uniqid = uniqid();
            if (!empty($user)) {
                 $user->id;
                  $recent_viewa = new RecentViewProduct;
                 $recent_viewa->user_id = $user->id;
                 $recent_viewa->product_id = $pid;
                 $recent_viewa->save();
            ...
            if($detailedProduct->digital == 1){
                return view('frontend.digital_product_details', compact('detailedProduct','user'));
            }
            else {
                return view('frontend.product_details', compact('detailedProduct','categoryOffers','user'));
            }
        }
        abort(404);
    }
    public function get_recent_view()
    {
         $user = Auth::user();
         $user->id;
        $flash_deal = DB::table('recent_view_products')
        ->where('user_id','=',24)
        ->get();
        if($flash_deal != null)
            return view('frontend.recent_products', compact('flash_deal'));
        else {
            abort(404);
        }
    }
    public function sugesion(Request $request)
    {
        $user = Auth::user();
        // $user->id;
        ...
        return view('frontend.user.customer.sugestion_criteria', compact('user','customer'));
        }

    }
    /*public function update(Request $request,$id)*/
    public function update_usertag(Request $request ,$id)
    {
        $customers = Customer::find($id);
        if (!empty($request->user_tags)) {
            $customers->user_tags = json_encode($request->user_tags);
        } else {
            $customers->user_tags = json_encode(array());
        }

        if($customers->save()){
            flash(__('Your tag has been updated successfully!'))->success();
            return redirect()->route('products.sugesion_criteria');
        }
